...
<!-- COMMENT CS: To me, it feels important to stress that (1) Consent must be *freely given* (Art. 4(11), 7(4)). In research this is often questionable where power asymmetries exist (students, employees, patients); mention that consent may then be invalid and another basis (e.g., Art. 6(1)(e) + a national research provisions) is preferable. (2) consent for research may be *broad* see Recital 33 (many researchers do not know this!!). -->
In practice, informed consent in research means telling participants what data you collect, why, how it will be stored, who will have access, and whether it will be shared or published. Also, mention what rights participants have and who they can contact in case of issues. If you plan to anonymize and share the data, this should be stated in the consent form. A useful tutorial on writing GDPR-compliant consent forms for research is provided by @Hallinan2023_InformationProvisionInformed. It is also helpful to tell participants about the intended level of anonymization and what it means for them (e.g., plausible deniability).
...
The randomized response technique is a technique that **transparently adds privacy at the stage of data collection** [@moshagen2012; @warner1965].
...
Randomized response is also an early example of what is now formalized as **differential privacy**—see the callout in [the chapter on perturbative techniques](../2_ANONYMISATION_TECHNIQUES/2_4_Perturbative.qmd#differential-privacy).
:::
...
<!-- COMMENT CS: The link looks broken on my end. Also, I would add one sentence here on Art. 30 records of processing: pseudonymization keys, their storage location, and access rights belong in the record (this is where most fail in practice) -->
For a practical, step-by-step workflow on when to remove direct identifiers and the pseudonym key across every copy of your data, see the [workflow callout in the chapter on personal data](1_3_Personal_Data.qmd#optimal-workflow).
...
<!-- COMMENT CS: The table's binary "No (re-identification is not reasonably possible)" for anonymization is fine, but I would add a footnote that this standard is *contextual and revisable*: data anonymous for one recipient may be personal data in the hands of another with different means (see recital 26 "by any person" wording, and the "famous" Breyer C-582/14 on relative identifiability). -->
...
<!-- COMMENT CS: I feel that aggregated neighborhood statistics can still leak when group sizes are small (differencing attacks, small-cell disclosure). Saying "no individuals can be singled out" without mentioning cell-size thresholds could be the wrong lesson. -->
