...
A dataset satisfies **k-anonymity** if, for every combination of quasi-identifiers (the indirect identifiers that could be used for linking), there are at least *k* records (of individuals) sharing that same combination. In other words: no one is unique, and no one stands out from a group smaller than *k*.
...
We will use k-anonymity practically in the [the next chapter](../2_ANONYMISATION_TECHNIQUES/2_1_Planning_Privacy) of this tutorial, where we calculate it using R and the `sdcMicro` package.
