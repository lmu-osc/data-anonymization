...
In general, it is worth to check the assumptions of the anonymization process you routinely adapt every few years or so, especially if new techniques have emerged (see callout box below).
...
Let's go back to the two `sdcObjects` we created in [the chapter on perturbative techniques](../anonymisation-techniques/perturbative): `sdc_micro` (where we applied microaggregation) and `sdc_noise` (where we applied noise).
