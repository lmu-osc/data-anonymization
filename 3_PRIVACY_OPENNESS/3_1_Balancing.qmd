...
sdc_nonpert <- readRDS("../sdc_nonpert.rds")
sdc_noise <- readRDS("../sdc_noise.rds") # Change based on the location of your Sdc-object file
sdc_micro <- readRDS("../sdc_micro.rds")
...
Let's go back to the two `sdcObjects` we created in [the chapter on perturbative techniques](#0): `sdc_micro` (where we applied microaggregation) and `sdc_noise` (where we applied noise).
...
As a last anonymization step, I will now **generalize rare religion categories** using `fct_lump_min()` from `forcats`. Since `religion` is not a variable in the `sdcObject`<!--why not though?-->, I apply this directly to the dataset as an additional layer, protecting individuals with rare religions.
