...
These data are treated differently by GDPR: By default, their processing is prohibited (Art. 9(1)), and, only when an exemption applies, allowed—for example, when processing is necessary for scientific purposes (Art. 9(2)(j)) and special saefeguards are in place (see [the chapter on mechanisms of data protection](1_4_Mechanisms)).
...
<<<<<<< HEAD:1_FOUNDATION/1_3_Personal_Data.qmd
...
## A Default Workflow for Handling Identifiers
>>>>>>> origin/main:foundations/personal-data.qmd
...
3.  **Remove the pseudonym key** using the same logic. If there are concrete reasons to keep identifiers or pseudonyms, and either would serve the purpose, keep the pseudonym key—it is the safer option—but store it *separately* from the data, with restricted access and a defined deletion date.

4.  **Store a secure copy** of the data with direct identifiers and pseudonyms that are really needed, with read-only access, e.g., on your institution's password-protected server. Delete the primary raw data.
