---
title: "Personal Data"
aliases: [1_FOUNDATION/1_3_Personal_Data.html]
...
- Distinguish between **personal** and **non-personal data**, as well as **sensitive** and **non-sensitive data**, and identify **direct** and **indirect identifiers**.
:::
...
- **Direct identifiers**. These are pieces of information that can directly point out and identify an individual, such as address, social security number, bank accounts, or an email address.
...
The GDPR ([Art. 9](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng#art_9)) describes several categories of **sensitive data** that receive heightened protection:
...
These data are treated differently by GDPR: By default, their processing is prohibited ([Art. 9(1)](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng#art_9)), and, only when an exemption applies, allowed—for example, when processing is necessary for scientific purposes ([Art. 9(2)(j)](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng#art_9)) and special safeguards are in place (see [the chapter on mechanisms of data protection](mechanisms.qmd)).

In practice, this means you need to be **extra careful** with these variables when collecting, storing, analyzing, and sharing data—they require stronger protection measures, and in some cases, it may be advisable to remove them from a shared dataset entirely if they are not essential to the research question.

**Information on criminal convictions and offenses** is a special case. Under [Art. 10](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng#art_10) GDPR, such data may only be processed under the control of an official authority. The GDPR leaves room for national exemptions, but Germany (and e.g., Bavaria) has not created a clear, general basis for processing this data in research. If your work requires collecting data on criminal convictions, I recommend talking to your institution's data protection officer.
...
| email | Email address of participant | What is your email address? | String of characters |
| plz | German postal code ("Postleitzahl") | What is your postal code? | String of characters |
...
| years_in_job | Number of years the participant has been in their current job | How many years have you been in your current job? | Integer; 0-100 |
:::
...
The `pol_immigration`, `pol_environment`, `pol_redistribution`, and `pol_eu_integration` variables capture **political opinions**, and `religion` captures **religious beliefs**. Both categories are explicitly listed as sensitive data under [Art. 9](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng#art_9) GDPR. These variables require **heightened protection** because their disclosure could lead to discrimination or other harm for the individuals involved.
:::
...
2.  **Store a secure copy** of the data with direct identifiers and pseudonyms that are really needed, with read-only access, e.g., on your institution's password-protected server. Delete the primary raw data.
...
4.  **Remove the pseudonym key** using the same logic. If there are concrete reasons to keep identifiers or pseudonyms, and either would serve the purpose, keep the pseudonym key—it is the safer option—but store it *separately* from the data, with restricted access and a defined deletion date.
...
Now, write the R code to delete all direct identifiers from the dataset.
...
write.csv(data_withoutdirectidentifiers, here::here("simulated-data-noidentifiers.csv"), row.names = FALSE)
```

Delete the old file—unless one of the retention purposes above applies. Direct identifiers are often collected for a reason (e.g., a name to honor a withdrawal request, or an email to return results or flag an incidental finding), so the rule is not "always delete immediately" but **"delete once the documented purpose is gone."** From a legal perspective, once the data is fully anonymized, access and deletion rights no longer apply (see [the chapter on mechanisms of data protection](mechanisms.qmd)). Communicate your timeline transparently to participants during participation in the study. 

Where a purpose genuinely requires keeping identifiers or a pseudonym key, store that piece separately from the analysis data, restrict access to it, and set a date to delete it.
:::
