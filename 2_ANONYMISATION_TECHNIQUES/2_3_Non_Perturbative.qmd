...
-   but: when publishing anonymization script: needs to be anonymous as well (e.g., not naming specific countries; see [the chapter on documentation](../3_PRIVACY_OPENNESS/3_2_Documentation))
...
`globalRecode()` cannot do this directly since `cut()` only handles continuous intervals - and "other" spans two separate ranges (00000-79999 and 90000-99999). Instead, I directly modify the `@manipKeyVars` slot of the sdcObject and recalculate risks:
