...
I then applied **top-coding to income**. My goal here is to protect the few individuals in the dataset with a very high income. I start by inspecting the data visually with a boxplot:
...
I see a few extreme outliers above 400 000 €. Choosing the 95th percentile at `r quantile(data_withoutdirectidentifiers$income, 0.95)`€ would cut these extreme values easily off, as well as some values around 100k. Let's start with going with that.

I'll use `topBotCoding` and set the value at the 95th percentile, replacing those values with that one.
...
As income is the only numeric variable in the `sdcObject`, the possible disclosure risk is reduced by those 5 percent points we cut off.

I also used **top-coding for years in job**. Again, I started by familiarizing myself with the data.
...
I decided to use 15 as a cut-off value, as this cuts-off the extreme values.

```{r}
sdc_nonpert <- topBotCoding(
  obj         = sdc_nonpert,
  column      = "years_in_job",
  value       = 15,        # values above 15 years get capped at 15
  replacement = 15,
  kind        = "top"
)
...
We see that k-anonymity values have not been influenced by the changes so far - this is primarily due to the plz variable that is unique for most participants. I therefore decided to generalize the postal code.

```{r}

extractManipData(sdc_nonpert)
...
The five regions map loosely to north, north-central, central, south-central, and south Germany.
