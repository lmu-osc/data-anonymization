...
![Record swapping: gender values are exchanged between records. The 1-order frequencies stay the same, but the 2-order (joint) frequencies can change.](../pictures/Swapping-cropped.png){fig-alt="A before-and-after table where gender values are swapped between records; frequency tables below show that 1-order frequencies stay equal while 2-order frequencies change."}
...
![Re-sampling: original ages are replaced with values drawn from bootstrap estimates.](../pictures/Re-sampling-cropped.png){fig-alt="A before-and-after table where the exact ages are replaced with re-sampled values."}
