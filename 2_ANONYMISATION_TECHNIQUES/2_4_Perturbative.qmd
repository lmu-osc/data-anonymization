...
In general, perturbation does not increase k-anonymity<!--why?-->. Rather, it adds another layer of protection, especially to continuous variables.
...
`addNoise` draws from a normal distribution with mean 0 and standard deviation = `noise × sd(income)` and adds it to each value. Every record gets a unique (slightly wrong) income. When `noise = 3.5`, we achieve a disclosure risk of up to 75.5%.<!--shouldn't this be down to?-->
