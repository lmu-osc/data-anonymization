...
Take another look at the dataset<!-- 'our dataset' if introduced in home apge?--> from [the chapter on personal data](../1_FOUNDATION/1_3_Personal_Data), the version without direct identifiers.<!--where identifiers were removed in the chapter on personal data?--> Try to answer the following questions:
...
Let's start by creating a sdcMicro object using the class `sdcMicroObj`<!--I dont know where the class of the object is provided, shouldn't you say 'using the function createSdcObj' instead? or both? or perhaps 'of the class' using sounds like I need to do something but I don't know what to do-->. The object becomes the working environment for all subsequent anonymization and risk assessment steps. It stores
...
In this exercise, you will assess privacy risks with `sdcMicro` using the example data from the [chapter on personal data](../1_FOUNDATIONS/1_3_Personal_Data). We will work with the dataset `data_withoutdirectidentifiers`.<!--using the same dataset loaded in the home page and modified in the exercise on the personal data page which removed direct identifiers ?--> 
