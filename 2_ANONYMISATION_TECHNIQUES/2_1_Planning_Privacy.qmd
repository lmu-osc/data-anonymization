...
The same logic applies to `religion`: if a plausible attacker is someone who knows the participant from a religious community, then `religion` belongs in `keyVars`, not treated as a sensitive outcome. **The variables you choose as keys should reflect the attack scenario you consider most realistic for your specific sample and context.** In this case, I think it is not likely to be known by an attacker; however, we will address `religion` again at a later stage to [protect very rare values](../3_PRIVACY_OPENNESS/3_1_Balancing.qmd#rare-religion-step).
:::
