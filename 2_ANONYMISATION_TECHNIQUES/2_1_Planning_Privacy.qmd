...
Data minimisation (or in German, Datensparsamkeit) referes to the principle of "No Data, No Risk" (or less data, less risk), meaning that the threat of privacy breaches is reduced with the reduction of the amount of data collected. Datensparsamkeit pushes the data collector to consider to think about the mimimum amount of data that is needed to get the job done. Reducing the amount of stored data can also reduce the treat of the data storage to become target of a privacy attack.
...
Federated learning and distributed data analysis needs to be planned in advance. This is because the algorithms for federate analysis are different from those that would be run on the entire data set, and because they need to be tested for **data shift** or **data drift**, hence that processing the data locally and then aggregating does lead to the same results as running the algorithm on the entire data set.
