...
De-associative techniques protect privacy by breaking the link between indirect identifiers and sensitive attributes, rather than altering the values themselves. The underlying idea is that even if an attacker can identify a person's record based on their demographic attributes, they should not be able to learn their sensitive values from it.
...
1.  Generalize the indirect identifiers to create buckets (e.g., exact countries → continents).
2.  De-generalize the identifiers within each bucket back to their original values (e.g., continents → countries again).
