...
aliases: [2_ANONYMISATION_TECHNIQUES/2_3_Non_Perturbative.html]
...
![Local recoding: only the rare religion values are generalized (here to "Christian"), while the rest stays unchanged.](../pictures/local-recoding-cropped.png){fig-alt="A before-and-after table where only two of the five records have their religion changed to Christian; the other records are unchanged."}

This means that values are on different levels (in this example: country vs. continent). Compared to global recoding, local recoding is more targeted and loses less information, but it can be harder to justify and document consistently—you need to be transparent about which categories were collapsed and why.
...
This helps when the bulk of the data is unremarkable and only a handful of extreme values are risky, because they belong to very few people. In the example dataset, a very high income might apply to only one or two participants, which makes them easy to single out. Capping income at the 95th percentile means that all top earners share the same reported value instead.

Top-and-bottom coding counts as non-perturbative: the capped value is not a wrong value, just a less precise one. It has to be read as "this much or more" rather than as an exact amount—which is why the cap needs to be documented as such (see the [note on labeling top-coded values](#top-coding-labels) below).
...
- **Variable suppression**: Drop a whole variable from the released dataset if it is too risky to include at all. Direct identifiers like name and email address are a clear case—I already did this in [the chapter on personal data](../foundations/personal-data).
...
I see that k-anonymity values have not been influenced by the recoding of age so far—this is primarily due to the `plz` variable that is unique for most participants. I therefore decided to **generalize the postal code**.
...
With 12 more suppressions on education and 4 on gender, I can achieve 3-anonymity. Only 29 entries remain that violate 5-anonymity. To me, this is enough protection on k-anonymity, while still preserving most of the utility, especially on the most interesting variables.
:::
...
The boxplot shows a few extreme outliers above 400 000 €. The 95th percentile, at `r format(round(quantile(data_withoutdirectidentifiers$income, 0.95)), big.mark = " ")` €, cuts these off. `topBotCoding()` takes two arguments for this: `value` sets the threshold, and `replacement` the value written in its place. I set both to the 95th percentile, so every income above it is reported as exactly that amount.
...
::: {#top-coding-labels .callout-warning collapse="false"}
## Labeling Top-Coded Values
...
- **Mark it in the data**, e.g., as "\>60" or "4+". This is unmistakable, but turns the variable into text and makes further analysis more work.
:::

Together, these steps bring the disclosure risk down to a maximum of 92%, compared to the original data.
