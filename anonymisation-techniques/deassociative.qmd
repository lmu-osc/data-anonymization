...
Let's implement anatomization step by step, following the algorithm proposed by @xiao2006. It is not available in `sdcMicro`, so we will build it ourselves with a lot of data wrangling. The algorithm has 19 steps, but I will break them down into more workable chunks.
...
You then try to slot each remaining participant into a group that has no one with the same sensitive value yet (so the group stays diverse, see the [eligibility condition](#eligibility-condition) above). If no such group exists, the record cannot be released safely and has to be suppressed.
